# analytics.py
# Handles Real-Time Data Analysis & Visualization
...
class TrafficAnalytics:
    ...
    def track_passed_vehicle(self, vehicle):
        """ Log data when a car successfully crosses the intersection """
        self.total_cars += 1
        self.total_wait_accumulated += (vehicle.waiting_time / 60) # frames to seconds approx

    def update(self, vehicles):
        """ Periodically sample traffic density for the live graph """
        self.update_timer += 1
    ...
    def draw_dashboard(self, ctx, traffic_state, emergency_stats):
        """ Renders the HUD over the simulation """
        ...
        direction = parts[0]; color_name = parts[1]
        ...
        ctx.font = "12px Consolas"; ctx.fillStyle = "#AAAAAA"
        ctx.fillText("TOTAL CARS", 25, 65); ctx.fillText("AVG WAIT", 125, 65); ctx.fillText("FLOW RATE", 225, 65)
        
        ctx.font = "bold 14px Consolas"; ctx.fillStyle = "#00FFCC"
        ctx.fillText(f"{self.total_cars}", 25, 85)
        
        if avg_wait < 5: ctx.fillStyle = "#00FF00"
        elif avg_wait < 15: ctx.fillStyle = "#FFFF00"
        ...
        # Draw Live Graph
        graph_x = 25; graph_y = 120; graph_w = 290; graph_h = 50
        ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(graph_x, graph_y, graph_w, graph_h)
        ctx.beginPath(); ctx.strokeStyle = "#FFFF00"; ctx.lineWidth = 2
