...
document.getElementById("loading").innerText = "Status: Online | PB-ACS Active"
...
class Vehicle:
    def __init__(self, start_dir):
        ...
        # Spawn Logic: Ambulance vs Car
        if random.random() < 0.015:
            self.type = 'AMBULANCE'
            self.color = "#FFFFFF" 
            self.speed = random.uniform(6.5, 8.5) 
            self.max_speed = self.speed
            self.width = 24; self.length = 45
        else:
            self.type = 'CAR'
            self.color = random.choice(["#E74C3C", "#3498DB", "#F1C40F", "#ECF0F1", "#111111"])
            self.speed = random.uniform(5.0, 7.0)
            self.max_speed = self.speed
            self.width = 22; self.length = 40
            
        self.intention = random.choices(['straight', 'left', 'right'], weights=[50, 25, 25])[0]
        if self.intention == 'left': self.lane_index = 0
        elif self.intention == 'right': self.lane_index = 1
        else: self.lane_index = random.choice([0, 1])
            
        lane_offset = LANE_WIDTH * 1.5 if self.lane_index == 0 else LANE_WIDTH * 0.5
            
        if start_dir == 'N': self.x = CENTER_X - lane_offset; self.y = CANVAS_HEIGHT + 60; self.vx, self.vy = 0, -1
        elif start_dir == 'S': self.x = CENTER_X + lane_offset; self.y = -60; self.vx, self.vy = 0, 1
        elif start_dir == 'E': self.x = -60; self.y = CENTER_Y - lane_offset; self.vx, self.vy = 1, 0
        elif start_dir == 'W': self.x = CANVAS_WIDTH + 60; self.y = CENTER_Y + lane_offset; self.vx, self.vy = -1, 0
    ...
    def check_car_ahead(self, all_vehicles):
        ...
        for other in all_vehicles:
            ...
            dist = 9999; is_ahead = False
            if self.start_dir == 'N' and other.y < self.y: dist = self.y - other.y; is_ahead = True
            elif self.start_dir == 'S' and other.y > self.y: dist = other.y - self.y; is_ahead = True
            elif self.start_dir == 'E' and other.x > self.x: dist = other.x - self.x; is_ahead = True
            elif self.start_dir == 'W' and other.x < self.x: dist = self.x - other.x; is_ahead = True
    ...
    def update(self, all_vehicles):
        ...
        if self.state == 'APPROACHING' and not should_stop:
            ...
            if 0 < dist < 120:
                ...
                if self.type == 'AMBULANCE' and current_dir_orange: should_stop = False
                elif current_dir_green: should_stop = False 
        ...
        if should_stop:
            ...
            if self.speed < 2: self.speed += 0.8 
            else: self.speed = min(self.max_speed, self.speed + 0.2)

        if self.state in ['APPROACHING', 'DEPARTING']:
            self.x += self.vx * self.speed; self.y += self.vy * self.speed
            ...
            if self.t >= 1.0:
                ...
                p_last = self.curve_points[-1]; p_prev = self.curve_points[-2]
    ...
    def start_turn(self):
        ...
        self.state = 'TURNING'; self.t = 0
        p0 = (self.x, self.y)
        out_lane_0 = LANE_WIDTH * 1.5; out_lane_1 = LANE_WIDTH * 0.5
        
        if self.start_dir == 'N':
            if self.intention == 'left': p3=(0, CENTER_Y+out_lane_0); p1=p2=(self.x, CENTER_Y+out_lane_0)
            elif self.intention == 'right': p3=(CANVAS_WIDTH, CENTER_Y-out_lane_1); p1=(self.x, CENTER_Y-100); p2=(CENTER_X+100, CENTER_Y-out_lane_1)
        elif self.start_dir == 'S':
            if self.intention == 'left': p3=(CANVAS_WIDTH, CENTER_Y-out_lane_0); p1=p2=(self.x, CENTER_Y-out_lane_0)
            elif self.intention == 'right': p3=(0, CENTER_Y+out_lane_1); p1=(self.x, CENTER_Y+100); p2=(CENTER_X-100, CENTER_Y+out_lane_1)
        elif self.start_dir == 'E':
            if self.intention == 'left': p3=(CENTER_X-out_lane_0, 0); p1=p2=(CENTER_X-out_lane_0, self.y)
            elif self.intention == 'right': p3=(CENTER_X+out_lane_1, CANVAS_HEIGHT); p1=(CENTER_X+100, self.y); p2=(CENTER_X+out_lane_1, CENTER_Y+100)
        elif self.start_dir == 'W':
            if self.intention == 'left': p3=(CENTER_X+out_lane_0, CANVAS_HEIGHT); p1=p2=(CENTER_X+out_lane_0, self.y)
            elif self.intention == 'right': p3=(CENTER_X-out_lane_1, 0); p1=(CENTER_X-100, self.y); p2=(CENTER_X-out_lane_1, CENTER_Y-100)
        self.curve_points = [p0, p1, p2, p3]

    def draw(self):
        ctx.save(); ctx.translate(self.x, self.y); ctx.rotate(self.angle)
        ctx.fillStyle = self.color; ctx.fillRect(-self.width/2, -self.length/2, self.width, self.length)
        ctx.fillStyle = "black"; ctx.fillRect(-self.width/2+2, -self.length/2+2, self.width-4, 10)
        
        if self.type == 'AMBULANCE':
            if int(window.performance.now() / 200) % 2 == 0:
                ctx.fillStyle = "red"; ctx.fillRect(-self.width/2, -5, self.width, 10) 
            else:
                ctx.fillStyle = "blue"; ctx.fillRect(-self.width/2, -5, self.width, 10) 
            ctx.fillStyle = "#D32F2F"; ctx.fillRect(-2, -8, 4, 10); ctx.fillRect(-5, -5, 10, 4)

        if self.state in ['APPROACHING', 'TURNING'] and int(window.performance.now()/250)%2==0:
            ctx.fillStyle = "#FF9800"
            if self.intention == 'left': ctx.fillRect(-self.width/2-2, -self.length/2, 4, 10)
            elif self.intention == 'right': ctx.fillRect(self.width/2-2, -self.length/2, 4, 10)
        ctx.restore()
...
def draw_background():
    ctx.fillStyle = "#4CAF50"; ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT) 
    ctx.fillStyle = "#333"; ctx.fillRect(CENTER_X - ROAD_WIDTH/2, 0, ROAD_WIDTH, CANVAS_HEIGHT) 
    ctx.fillRect(0, CENTER_Y - ROAD_WIDTH/2, CANVAS_WIDTH, ROAD_WIDTH) 
    
    ctx.strokeStyle = "#F1C40F"; ctx.lineWidth = 4; ctx.setLineDash([])
    ...
    ctx.strokeStyle = "white"; ctx.lineWidth = 2; ctx.setLineDash([15, 20])
    ctx.beginPath(); ctx.moveTo(CENTER_X - LANE_WIDTH, 0); ctx.lineTo(CENTER_X - LANE_WIDTH, CENTER_Y - ROAD_WIDTH/2); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(CENTER_X - LANE_WIDTH, CENTER_Y + ROAD_WIDTH/2); ctx.lineTo(CENTER_X - LANE_WIDTH, CANVAS_HEIGHT); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(CENTER_X + LANE_WIDTH, 0); ctx.lineTo(CENTER_X + LANE_WIDTH, CENTER_Y - ROAD_WIDTH/2); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(CENTER_X + LANE_WIDTH, CENTER_Y + ROAD_WIDTH/2); ctx.lineTo(CENTER_X + LANE_WIDTH, CANVAS_HEIGHT); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(0, CENTER_Y - LANE_WIDTH); ctx.lineTo(CENTER_X - ROAD_WIDTH/2, CENTER_Y - LANE_WIDTH); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(CENTER_X + ROAD_WIDTH/2, CENTER_Y - LANE_WIDTH); ctx.lineTo(CANVAS_WIDTH, CENTER_Y - LANE_WIDTH); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(0, CENTER_Y + LANE_WIDTH); ctx.lineTo(CENTER_X - ROAD_WIDTH/2, CENTER_Y + LANE_WIDTH); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(CENTER_X + ROAD_WIDTH/2, CENTER_Y + LANE_WIDTH); ctx.lineTo(CANVAS_WIDTH, CENTER_Y + LANE_WIDTH); ctx.stroke()

    ctx.fillStyle = "rgba(0, 100, 255, 0.1)"; ctx.strokeStyle = "rgba(0, 100, 255, 0.5)"; ctx.setLineDash([])
    ctx.fillRect(CENTER_X - ROAD_WIDTH/2, CANVAS_HEIGHT - 300, ROAD_WIDTH/2, 290); ctx.strokeRect(CENTER_X - ROAD_WIDTH/2, CANVAS_HEIGHT - 300, ROAD_WIDTH/2, 290)
    ctx.fillRect(CENTER_X, 10, ROAD_WIDTH/2, 290); ctx.strokeRect(CENTER_X, 10, ROAD_WIDTH/2, 290)
    ctx.fillRect(10, CENTER_Y - ROAD_WIDTH/2, 290, ROAD_WIDTH/2); ctx.strokeRect(10, CENTER_Y - ROAD_WIDTH/2, 290, ROAD_WIDTH/2)
    ctx.fillRect(CANVAS_WIDTH - 300, CENTER_Y, 290, ROAD_WIDTH/2); ctx.strokeRect(CANVAS_WIDTH - 300, CENTER_Y, 290, ROAD_WIDTH/2)

    ctx.lineWidth = 6; m = ROAD_WIDTH / 2
    ctx.strokeStyle = get_light_color('N'); ctx.beginPath(); ctx.moveTo(CENTER_X - m, CENTER_Y + m); ctx.lineTo(CENTER_X, CENTER_Y + m); ctx.stroke()
    ctx.strokeStyle = get_light_color('S'); ctx.beginPath(); ctx.moveTo(CENTER_X, CENTER_Y - m); ctx.lineTo(CENTER_X + m, CENTER_Y - m); ctx.stroke() 
    ctx.strokeStyle = get_light_color('E'); ctx.beginPath(); ctx.moveTo(CENTER_X - m, CENTER_Y); ctx.lineTo(CENTER_X - m, CENTER_Y - m); ctx.stroke() 
    ctx.strokeStyle = get_light_color('W'); ctx.beginPath(); ctx.moveTo(CENTER_X + m, CENTER_Y); ctx.lineTo(CENTER_X + m, CENTER_Y + m); ctx.stroke() 

def collect_sensor_data():
    """ Simulates Object Detection / YOLOv8 parsing """
    ...
    while True:
        ...
        if random.random() < 0.08: 
            ...
            if is_spawn_clear(new_dir, lane_idx):
                v = Vehicle(new_dir)
                v.intention = intention; v.lane_index = lane_idx
        ...
        # Core Architecture: Ask AI for instruction
